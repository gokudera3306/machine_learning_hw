...
class Classifier(nn.Module):
    def __init__(self, concat_nframes, output_dim=41, hidden_layers=1, hidden_dim=256):
        super(Classifier, self).__init__()

        self.concat_nframes = concat_nframes
        self.hidden_size = hidden_dim

        self.lstm = nn.LSTM(39, hidden_dim, batch_first=True, bidirectional=True, num_layers=4, dropout=0.4)
    ...
    def forward(self, x):
        ...
        mid_index = self.concat_nframes//2  # 擷取中間的 Feature 是參考右側連結： https://blog.csdn.net/qq_43613342/article/details/127007955
        out = out[:, mid_index - 5: mid_index + 5]
...
model = Classifier(concat_nframes, hidden_layers=hidden_layers, hidden_dim=hidden_dim).to(device)
